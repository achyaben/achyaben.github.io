<template>
  <div v-if="show" class="fixed bottom-0 left-0 right-0 z-[999] p-4 transition-transform duration-500 transform translate-y-0">
    <div class="bg-gray-900 text-white rounded-2xl shadow-2xl p-5 border border-gray-700 max-w-lg mx-auto relative overflow-hidden">
      <!-- Background pattern -->
      ...
      <div class="flex items-start gap-4">
        <div class="bg-primary/20 p-2 rounded-xl">
          <img src="/assets/achyaben-logo.svg" alt="App Logo" class="h-10 w-10 rounded-lg shadow-sm" />
        </div>
        ...
        <button @click="close" class="text-gray-400 hover:text-white absolute top-3 right-3 transition-colors">
      ...
      <div class="mt-4 pt-4 border-t border-gray-700/50">
        <div v-if="platform === 'ios'" class="flex items-center justify-center gap-2 text-xs font-medium">
          <span>下部の</span>
          <div class="bg-gray-800 p-1.5 rounded-lg border border-gray-700">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <path stroke="currentColor" stroke-width="2" d="M4 12V20a2 2 0 0 0 2 2H18a2 2 0 0 0 2-2V12 M12 3V15 M12 3 L8 7 M12 3 L16 7" />
            </svg>
          </div>
          <span>を押し、 「ホーム画面に追加」をタップ</span>
        </div>
        <div v-else class="flex flex-col gap-2">
          <button @click="installAndroid" class="w-full bg-white text-gray-900 font-bold py-2 rounded-xl text-xs hover:bg-gray-100 transition shadow-inner">
            今すぐ追加
          </button>
          <p class="text-[10px] text-gray-500 text-center">メニューの「アプリをインストール」からも追加可能です</p>
        </div>
      </div>
    </div>
  </div>
