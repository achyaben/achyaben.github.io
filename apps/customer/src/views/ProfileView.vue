<template>
  <div class="min-h-screen bg-gray-50 pb-24">
    ...
    <main class="container mx-auto px-4 py-6 max-w-lg">
      ...
      <div v-if="authProvider" class="mb-6 bg-white rounded-xl shadow-sm p-4 flex items-center border-l-4"
        :class="authProvider === 'line' ? 'border-green-500' : 'border-blue-500'">
        <div class="mr-3">
          <img v-if="authProvider === 'line'" src="https://upload.wikimedia.org/wikipedia/commons/4/41/LINE_logo.svg"
            alt="LINE" class="w-8 h-8" />
          <img v-else-if="authProvider === 'google'"
        ...
        <div class="ml-auto">
          <span
            class="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 border border-gray-200">認証済み</span>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-6 space-y-6">
        ...
        <form @submit.prevent="saveProfile" class="space-y-6">
          ...
          <div>
            ...
            <input v-model="form.phone" type="tel" required pattern="^[0-9]{10,11}$" placeholder="0312345678"
              class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary" />
          </div>
          ...
          <div class="border-t pt-4 space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">市区町村・番地・建物名・フロア</label>
              <textarea v-model="form.address" rows="3" placeholder="江戸川区南葛西5-13-10 ヤキベンビル 3F 会議室"
          ...
          <div class="pt-10 border-t border-red-100">
            <div class="bg-red-50 p-6 rounded-2xl border border-red-100">
              ...
              <button type="button" @click="handleDeleteAccount" :disabled="isLoading"
...
async function loadProfile() {
  try {
    ...
    if (data) {
      authProvider.value = user.app_metadata.provider || 'google';
